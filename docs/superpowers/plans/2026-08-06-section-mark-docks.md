...
## Task 7 — المهمّة ٧: البوّابة تقيس ما يُرسم لا ما يُوضَع

**سببها:** المهمّة ٦ أثبتت — قراءةً للكود وتجريبيًّا (طرفا مدى `FLAT` يعطيان
رقمًا متطابقًا بتًّا ببت) — أن `sweep()` تختبر هندسة `<polygon>` عبر
`isPointInFill` **ولا تقرأ `opacity` في أي سطر**. فبوّابةُ العتامة عمياء
عنها بالتصميم.

**وتحقّقٌ مستقلّ من المنسّق** عند أسوأ نقطة (390، `y=1239`): عتامة الأضلاع
`[1, 1, 1, 0, 0.579, 1]` — **الضلع 3، الذي أبلغت البوّابة أنه يغطّي 37.9%
من «من نحن؟»، عتامته صفرٌ بالضبط.** أي أن الرقم يخصّ ضلعًا غير مرئيّ. وفي
الحالات الثابتة الثلاث العتامة `1` في الستّة كلّها — العلامة تُرى حيث يجب.

**هذا تصحيحُ أداةٍ لا تخفيفُ عتبة:** ضلعٌ شفّافٌ تمامًا **لا يغطّي شيئًا**.
البوّابة التي تعدّه مغطّيًا تقيس الوهم. والضلع نصف الشفّاف **يغطّي فعلًا**
فيبقى محسوبًا — لا استثناء له.

**الملفات:**
- تعديل: `scripts/audit.js` — `sweep()` وحدها

- [ ] **الخطوة ١: سجّل الحالة**

الصق `scripts/audit.js` ثم `__audit.sweep(61)` على 390×844. سجّل `worst`.

- [ ] **الخطوة ٢: اجعل `inAny` تحترم العتامة**

داخل `sweep()`، استبدل جسم `inAny` بالآتي:
...
⚠️ `closest` على عنصر SVG مدعومٌ في المتصفّحات الحديثة؛ الحارس `if (!host)`
يُبقي السلوك القديم إن غاب، فلا ينكسر الفحص صامتًا.

- [ ] **الخطوة ٣: أثبت أن الفحص ما زال يميّز**

**قبل أي احتفال:** أوقِف البوّابة مؤقّتًا في المتصفّح بجعل كل الأضلاع
مرئيّة، وتأكّد أن `sweep()` **ترجع للرسوب**:

```js
document.querySelectorAll('[data-mark-shard]').forEach(function (s) {
  s.style.setProperty('opacity', '1', 'important');
});
__audit.sweep(61)
```
المتوقّع: `pass: false` بأرقامٍ قريبة من القديمة. فحصٌ ينجح هنا = فحصٌ
كُسر لا فحصٌ صُحّح. ثم أعِد التحميل لإزالة التجاوز.

- [ ] **الخطوة ٤: المسح الحقيقي**

`__audit.sweep(61)` على **390 و1280** في الوضعين، بعد `navigate` كامل
وانتظار استقرار. سجّل الأربعة بأرقامها.

إن بقي رسوب: **لا تلمس العتبة 0.05 ولا العيّنة ولا نقاط التمرير.** أبلِغ
`DONE_WITH_CONCERNS` بأرقامٍ كاملة، ومعها عتامةُ الضلع المُبلَّغ عنه عند
نقطة الرسوب — فذلك يفصل التغطية الحقيقيّة عن الوهم.

- [ ] **الخطوة ٥: تحقّق نهائي وارتكاب**
...
```bash
git add scripts/audit.js
git commit -m "fix: البوّابة تحترم العتامة — الضلع الشفّاف لا يغطّي شيئًا"
```

---

## Task 8 — المهمّة ٨: المعيار يميّز الجوار من العبور · وخفض `FLAT`

**سببها:** معيار القبول ٣ («صفر تغطية») **خاطئٌ في أصله** — يمنع ما يفعله
التصميم عمدًا: وضعُ العلامة بجوار النصّ الذي تخصّه.

**الدليل المقيس (جولة ٣ من المهمّة ٦):** عند 1280 و`y=1535` قيمة `t1i`
للضلع 0 هي **0.9636** — الضلع عند 96% من رحلته نحو مرساه. ومرساه هو الفاصل
الزخرفيّ **داخل بطاقة «الهدف» نفسها**، على بُعد 16px من عنوانها. فأيّ ضلعٍ
يقترب من مرساه — كما يجب — يمرّ قرب عنوان بطاقته، وعتامته حينها **يجب** أن
ترتفع لأن شرط الرسوّ عتامة 1. تقاربٌ هندسيّ حتميّ لا عطل.

وهو **نفس الصنف** الذي حُكم ببراءته سابقًا في «منذ عام 2013» (3.2%، سكون).

**القاعدة الصحيحة — دلاليّة لا رقميّة:**
> يُسمح بالتغطية حين يشترك **النصّ ومرسى الضلع المغطّي** في أقرب سلفٍ من
> `section` أو `li` — فالعلامة عندئذٍ بجوار نصٍّ تخصّه. ويُمنع حين يكون
> السلف المشترك `main` أو أعلى — فذلك ضلعٌ يعبر قسمًا لا شأن له به.

لا عتبةَ بكسلٍ سحريّة: المعيار بنيةُ المستند نفسها.

**الملفات:**
- تعديل: `scripts/audit.js` — `sweep()`
- تعديل: `src/components/site/mark-morph.tsx` — قيمة `FLAT` وحدها

- [ ] **الخطوة ١: اجعل `inAny` تُرجع الضلع لا `true`**

`sweep()` تحتاج معرفة **أيّ** ضلعٍ غطّى لتجد مرساه. غيّر `inAny` لتُرجع
فهرس الضلع أو `-1`، وحدّث نداءها في الحلقة.

- [ ] **الخطوة ٢: أضِف قاعدة الجوار**
...
وفي حلقة العدّ: لا تَعُدّ الإصابة إن كانت `neighbourly(hit, el)` صادقة.
واحتفظ بعدّادٍ منفصل `neighbour` لعدد الإصابات المسموحة، ليظهر في المُخرَج
فلا يختفي المسموح صامتًا.

- [ ] **الخطوة ٣: أثبت أن الفحص ما زال يميّز — مُلزمة**

القاعدة الجديدة يجب أن **ترسب** حين يغطّي ضلعٌ نصًّا في قسمٍ آخر. أثبِتها
بنقل مرسًى مؤقّتًا إلى قسمٍ غير قسمه في المتصفّح:

```js
var d = document.querySelector('[data-mark-dock="0"]');
document.querySelector('#about').appendChild(d);
__audit.sweep(61)
```
المتوقّع: `pass: false` — الضلع 0 صار مرساه في `#about` بينما يغطّي نصّ
`#pillars`، فلا جوار. ثم أعِد التحميل لإزالة النقل.
**فحصٌ ينجح هنا = فحصٌ كُسر لا فحصٌ صُحّح.** أرفِق أرقام هذه الخطوة.

- [ ] **الخطوة ٤: اخفض `FLAT` إلى أدناه الصالح**

في `mark-morph.tsx`، جرّب `FLAT` نازلًا من 0.85 بخطوة 0.05 حتى **0.30**،
وسجّل كل قيمةٍ بأرقامها على 390 و1280 في الوضعين. **أبلِغ أدنى قيمةٍ تُبقي
`worst.pct === 0`** بالقاعدة الجديدة — الهدف استعادة إحساس الرحلة: عند 0.85
الضلع مرئيّ في أوّل 7.5% وآخر 7.5% فقط، فيبدو ظهورًا واختفاءً لا انتقالًا.
كلّما قلّ `FLAT` طالت مدّة ظهور الضلع في رحلته.

- [ ] **الخطوة ٥: الحرّاس والارتكاب**

الحالات الثلاث الثابتة عتامتها 1 · `__audit.docks()` تبقى `pass: true` ·
`tsc`/`lint`/`build` أصفار. ثم ارتكابان منفصلان (الحصّالة، ثم `FLAT`).

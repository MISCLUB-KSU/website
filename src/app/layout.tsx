...
/**
 * خطُّ العناوين — **وزنان لا ثلاثة** (١٥ أغسطس ٢٠٢٦، بطلب الإدارة).
 *
 * حُذف `500`. كان يُحمَّل ملفًّا كاملًا (~٣٠ كيلو من ٢٩٦) لأجل **موضعين**:
 * فقرةِ الرئيسية البارزة، ورقمِ الترتيب في فهرس المشاريع. وكلاهما نُقل
 * صراحةً إلى `600` في موضعه — لا يُترك للمتصفّح يختار.
 *
 * ⚠️ **والتصريحُ مقصود.** لو حُذف الوزن وبقيت الأصناف `font-medium`،
 * لطبّق المتصفّح خوارزميةَ المطابقة: يبحث تحت ٥٠٠ فلا يجد (٤٠٠ غيرُ
 * محمَّل لهذا الخطّ)، ثم فوقها فيأخذ ٦٠٠ — النتيجةُ نفسُها **على متصفّحٍ
 * يلتزم المواصفة**. لكن محرّكاتٍ أخرى تصطنع الوزن بتغليظٍ رقميّ فيخرج
 * الحرفُ العربيُّ متورّمًا. فالصنفُ يقول ما يُرسَم.
 *
 * وخطُّ المتن يبقى بأوزانه الأربعة — مفحوصٌ أنها مستعملةٌ كلُّها.
 */

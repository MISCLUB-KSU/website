...
/**
 * ما يُسمح بضبطه اليوم — **ولا رفضَ فيها**.
 *
 * ⛔ **الرفضُ مقفولٌ عمدًا حتى يصل السلّم (`stage`).** الرفضُ اليوم يُكتب
 * `rejected` نهائيًّا، والصفُّ لا يحفظ «رفضَه قائدُ رغبته الأولى» — فمتى
 * أُضيف السلّم بعد يومين كان هؤلاء **خارجَه أصلًا**، ولا سبيلَ إلى معرفة
 * من هم لينزلوا إلى رغبتهم الثانية. فمن لا يريده القائدُ اليوم يتركه
 * «جديدًا» وحسب، وهو كافٍ لتبدأ المقابلات.
 *
 * ويُفحص في `actions.ts` أيضًا: إخفاءُ زرٍّ ليس منعًا.
 */
export const PHASE_ONE_STATUSES: readonly string[] = [
  "new",
  "reviewing",
  "accepted",
];

...
/**
 * **إرشادُ المقابلة انتقل إلى `content/capacity.ts` — ولم يعُد رقمًا واحدًا.**
 *
 * ⚠️ كان هنا `INTERVIEW_CAP = 15` لكلّ الجهات، وقد ناقض سقوفَ القبول يومَ
 * وُضعت (وحدةُ الميزانية تقبل ٤ و MISthon يقبل ٥٠). فصار يُشتقّ من سقف
 * الجهة: `interviewGuide(value)`. و**على الجهة لا على القائد** كما كان —
 * رئيسُ لجنةٍ نطاقُه ثلاثُ وحدات يرى ثلاثةَ عدّادات، وواحدٌ له يعني أن
 * تبتلع وحدةٌ نصيبَ أختها.
 */
export { interviewGuide, INTERVIEW_MARGIN } from "@/content/capacity";

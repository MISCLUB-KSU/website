...
import { splitAnswer } from "@/content/questions";
import { isolateLatin } from "@/lib/bidi";
import { useHydrated } from "@/lib/use-hydrated";
import { rosterCsv } from "./roster-csv";
import { askedQuestions, completeness, type Item } from "./scoring";
...
/**
 * **ترتيبُ الخيار كما يُقال للقائد** — «أنت خيارُه الثاني».
 *
 * ⚠️ **غيرُ `STAGE_LABELS`، وعمدًا.** تلك تسمّي الرتبةَ حين تكون هي
 * الموضوع («رغبةٌ ثانية» فوق رقاقة ترشيح)، وهذي تُركَّب في جملةٍ يقرؤها
 * القائدُ عن نفسه. وجملةٌ تقول «رغبةٌ ثالثة عنده» تُقرأ مرّتين قبل أن
 * تُفهم — رُصد ذلك على شاشةٍ حقيقيّة.
 */
const RANK_ORDINAL = ["", "الأوّل", "الثاني", "الثالث"];
              ...
              >
                ...
                {row.stage > 1 && (
                  <span
                    ...
                    title={`أنت خيارُه ${RANK_ORDINAL[row.stage]} — ما اختارك أوّلًا`}
                  >
        ...
        <header className="tile-ink fade-up relative rounded-b-none px-s5 py-s5 sm:px-s6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-x-s5 gap-y-s4">
            <div className="min-w-0">
              ...
              {/* ⚠️ **الرتبةُ تُقال، ولا تُترك للقائد يحزرها.** من لم
                  يخترك أوّلًا وقراءتُه كأنه فاضل بينك وبين غيرك واختارك
                  تظلمه وتظلم قرارَك.

                  ⚠️ **وتُقال بلغته لا بلغة النظام.** كانت: «نزل إليك —
                  رغبةٌ ثالثة عنده، وأولاه …»، وقيلت للرئاسة فردّت: «هذي
                  مب مفهومة». ثلاثةُ مصطلحاتٍ داخليّة في سطرٍ واحد —
                  «نزل إليك» و«عنده» و«أولاه» — ولا واحدٌ منها يعني شيئًا
                  لمن لم يقرأ الشيفرة. فصارت جملتين: من أنت عنده، ومن
                  اختاره هو. */}
              {row.stage > 1 && (
                <p
                ...
                >
                  ...
                  />
                  أنت خيارُه {RANK_ORDINAL[row.stage]} — وخيارُه الأوّل:{" "}

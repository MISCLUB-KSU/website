 ...
 *
 * ⚠️ **الخانات الستّة ليست صناديق متطابقة — عمدًا.** أضلاع الشعار الستّة
 * (`MARK_BOXES`) لها ثلاث نسب عرضٍ إلى ارتفاعٍ مختلفة (≈1.49 و0.80 و1.15).
 * صندوقٌ واحد ثابت المقاس للجميع (كما كان: 30×38 لكل خانة) يجبر
 * `mark-morph.tsx` على `scale(w/b.w, h/b.h)` بمعاملين مختلفين على
 * المحورين، فيَسحق الضلعَ ويُحرِّف زاويته الحادة عن ٢٤° — قِيس فعليًّا:
 * خمسٌ من الستّ هبطن بزوايا 13.4°/13.4°/13.4°/24.1°/17.3°/17.3° (الرابعة
 * وحدها نجت صدفةً، لأن نسبتها الحقيقية 0.799 قريبةٌ من نسبة الصندوق
 * الثابت 30/38=0.789). كل خانةٍ هنا مقاسها من نسبة ضلعها هو، فيبقى
 * `scale` واحدًا على المحورين والزاوية ٢٤° سليمة — ساكنةً وهابطة.
 *
 * ⚠️ العلامة ليست حاملة معنى — ترتيب المشروع في الرقيمة، واسمه في النصّ.
 * فهي `aria-hidden` ولا يُنقل بها معنى.
 */

/** ارتفاعٌ ثابت لكل خانة؛ العرض يُشتقّ من نسبة ضلعها فيبقى القصّ ٢٤° بالضبط
    مهما اختلفت الأبعاد — انظر تعليق `runPct` أسفل. */

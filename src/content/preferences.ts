...
/**
 * ⚠️ **ما لا تُستقبَل طلباتُه لا تُبنى له بطاقةُ اختيار — والمانعُ اليوم
 * `applicationState` لا `isExternal`.**
 *
 * كان `isExternal` هو المُقصي، وكانت `learnx` وحدها تحمله: تُعرض بطاقةً في
 * مجموعةٍ اسمها «المبادرات» فيختارها الطالب رغبةً كأيّ مشروع، وتسجيلُها
 * ليس عندنا — فالبطاقةُ تعِد بما لا نملكه، ومن يختارها يُنفق رغبةً من ثلاث
 * على بابٍ لا يفتحه هذا النموذج.
 *
 * ثم صُحِّح التصنيف (١٦ أغسطس ٢٠٢٦): `learnx` مبادرةُ النادي لا ضيفٌ عليه،
 * فرُفعت الراية — **ولم يتبدّل شيءٌ في النموذج**، لأن الإقصاء انتقل قبلها
 * إلى `applicationState: "closed"` وهو القفلُ الحقيقيّ (`OPEN_PREFERENCES`
 * أدناه). ويبقى الفلترُ هنا على `isExternal` لمبادرةٍ ليست لنا تأتي يومًا —
 * ولا واحدةَ اليوم، فهو لا يُقصي أحدًا.
 *
 * والإقصاءُ من مكانٍ واحدٍ يكفي، فكلُّ ما يليه مشتقٌّ منه: البطاقات، والقوائم
 * المنسدلة بلا جافاسكربت، ودليلُ التعريف في `preference-guide.tsx`، والتحقّق
 * على الخادم عبر `PREFERENCE_VALUES`، والروابطُ المباشرة عبر
 * `findDirectTarget` — يتوقّف كلُّه معًا ولا يبقى بابٌ خلفيّ.
 *
 * ⚠️ **ولم تُمسّ بيانات المبادرات في `projects.ts`** — تبقى في صفحة المشاريع
 * وفي الإنجازات والرعاة. وصفحاتُها تقرأ `PROJECTS` لا هذي القائمة، وهي اليوم

 ...
 * نفسِها.
 *
 * ⚠️ **وقد كان عدّادًا، فصار قفلًا في ٣١ أغسطس ٢٠٢٦** بطلب الرئاسة نصًّا:
 * «خلّ إجباري، محد يقبل فوق السقف اللي أنا حاطه». وكان مكتوبًا هنا أن
 * منعًا صارمًا «يعني قائدًا يقف أمام زرٍّ معطَّلٍ في ليلة الحسم بلا من يفتحه
 * له» — والاعتراضُ صحيحٌ ولم يُلغَ، بل عولج: الزرُّ يُعطَّل **ومعه سطرٌ
 * يسمّي الجهةَ ويقول كم من كم ومن يرفع السقف**.
 *
 * ⚠️ **والقفلُ في القاعدة لا في هذا الملفّ.** جدولُ `public.capacity`
 * وخطّافُ `applications_capacity_update` هما الحكم: يفحصان على مستوى
 * الجملة (فلا تمرّ دفعةٌ من عشرةٍ على مقعدٍ واحدٍ باقٍ) وبقفلٍ استشاريٍّ
 * لكلّ جهة (فلا يمرّ قائدان معًا على آخر مقعد). وما هنا نسخةٌ للعرض
 * وحدَه — **فأيُّ تعديلٍ لرقمٍ هنا يلزمه تعديلُ صفّه في الجدول، وإلّا
 * عرضت الشاشةُ سقفًا والقاعدةُ حكمت بغيره.**
 */

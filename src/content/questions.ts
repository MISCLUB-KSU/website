...
export type CustomQuestion = {
  ...
  /**
   * نصٌّ شفّافٌ **داخل** المربّع — يختفي أوّل ما يكتب الطالب.
   *
   * ⚠️ **ليس بديلًا عن `hint` بل لدورٍ آخر.** التلميح يبقى مقروءًا بعد
   * الكتابة فيصلح للقيد الذي يُراجَع («من ٣ إلى ٥ أسطر»)؛ والنائب يختفي
   * فلا يوضع فيه إلّا ما يُقرأ مرّةً قبل البدء — مثالٌ أو تأطيرٌ للسؤال.
   * ووضعُ شرطٍ فيه وحده يعني أن الطالب لا يراه وهو يراجع إجابته.
   */
  placeholder?: string;
...
/* ⚠️ **كان هنا `ANSWER_MAX = 400`** — سقفُ طولِ الإجابة النصّية، مُعلَّلًا
   بأن «سؤال القائد وسيلة فرز لا مقال». ورُفع مع بقيّة حدود الطول في ١٥
   أغسطس ٢٠٢٦ بقرار الإدارة — والتعليل كاملًا في `registration.ts`.
   والسؤالُ المطلوب يبقى مطلوبًا: يفحصه `question.required` على **الفراغ**
   لا على الطول. */

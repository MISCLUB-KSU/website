...
/**
 * حدُّ المرفق الواحد — **ميجابايتان**.
 *
 * ⚠️ **خُفض من خمسةٍ إلى اثنين (١٥ أغسطس ٢٠٢٦) بحسابِ سعةٍ لا بذوق.**
 * مستودعُ Supabase على الخطّة المجّانية **غيغابايتٌ واحد**، والموسمُ المتوقَّع
 * **أكثر من ٦٠٠ طلب**. وسقفُ الطلب الواحد كان ١٦ ميجا (سيرة ٥ + مشاريع ٥ +
 * ثلاثة مرفقات × ٢)، أي **٩٫٦ غيغا** لو ملأه الجميع — عشرةُ أضعاف المتاح.
 *
 * ⚠️⚠️ **والثقبُ الحقيقيّ ليس PDF بل الصورة.** `CV_TYPES` يقبل `image/png`
 * و`image/jpeg`، وطالبٌ يصوّر سيرته بجوّاله يرفع **٣–٥ ميجا**؛ ومئتا صورةٍ
 * كهذي تبتلع الغيغا وحدها. وسيرةُ PDF عادةً ٣٠٠–٨٠٠ كيلو، فلا يمسّها الحدُّ
 * الجديد أصلًا.
 *
 * ولم تُحذف الصيغُ الصُّوريّة — وهو الخيار الآخر الذي طُرح: كثيرٌ من الطلبة
 * لا حاسوبَ عنده، ومنعُ الصورة يمنعه من التقديم. وميجابايتان تكفيان صورةَ
 * صفحةٍ مقروءةً بعد ضغطِ الجوّال المعتاد.
 *
 * ⚠️ **وتغييرُ هذا الرقم يجرّ ثلاثة معه** — مربوطةٌ ولا تُكتشف بالبناء:
 *   ١) `bodySizeLimit` في `next.config.ts` — مشتقٌّ من `UPLOAD_BUDGET_BYTES`
 *   ٢) `file_size_limit` على مستودع `cv` في Supabase — يحتاج migration
 *   ٣) نصوصُ التلميح في `step-questions.tsx` — مشتقّةٌ من الثابت لا مكتوبةٌ رقمًا
 */
export const CV_MAX_BYTES = 2 * 1024 * 1024;

/** الحدُّ بالميجابايت — لنصوص الواجهة، فلا يُكتب الرقمُ بيدٍ في موضعين */
...
export function validateCvFile(file: unknown): string | undefined {
  ...
  if (file.size > CV_MAX_BYTES) {
    /* ⚠️ الرقمُ مشتقٌّ لا مكتوب: كان `5` نصًّا فلمّا خُفض الحدّ صار يكذب */
    return `حجم الملف أكبر من ${CV_MAX_MB} ميجابايت — اضغطه أو ضع رابطًا بدلًا عنه`;
  }
...
export type RegistrationState = {
  ...
  /**
   * ما وصل الخادمَ من المرفقات — يُعرض في شاشة النجاح.
   *
   * ⚠️ **وسببُه أن ضياع المرفق كان صامتًا على الطرفين** (١٥ أغسطس ٢٠٢٦):
   * `uploadOne` يخرج بلا سطرٍ في السجلّ حين يصل الحقلُ فارغًا، وشاشةُ
   * «وصل طلبك» لا تذكر المرفق أصلًا. فقدّم طالبٌ وسيرتُه لم تصل، ولم يعلم
   * هو ولا نحن. فالآن **تُذكر صراحةً**: وصلت أو لم تصل.
   */
  received?: { cv: boolean; projects: boolean };
};
